...
@section('main')
       <section class="column p-10  w-full flex-auto justify-center">
        {{-- <form action="{{ url('users/post/login/process') }}" method="POST" onsubmit="PostRequest(event,this,MyFunc.call_back)" class="w-full border-1 border-color-silver max-w-500 align-center br-10 bg-white  column g-5"> --}}
          <form style="backdrop-filter: blur(20px);" action="{{ url('users/post/coupon/checker/process') }}" method="POST" onsubmit="document.querySelector('.coupon-summary').classList.add('display-none');PostRequest(event,this,MyFunc.call_back);"  class="w-full max-w-500 align-center br-10 bg-primary-transparent c-white column g-5">
            <div class="column w-full p-10 align-center g-5">
                <span class="desc c-primary" style="font-family:luckiest guy;font-size:2rem;">Coupon Checker</span>
             <span>Enter a coupon code to check its status here</span>
          <div class="w-full column g-2">
             <input type="hidden" class="input" name="_token" value="{{ csrf_token() }}">
           
              <div style="border:none;border-bottom:1px solid" class="cont w-full h-50 border-1 border-color-silver">     <input autocomplete="off" readonly onfocus="this.removeAttribute('readonly')" type="text" placeholder="Enter coupon code" name="coupon" class="inp input required w-full h-full no-border bg-transparent br-10">
            </div>
          ...
          <div class="w-full display-none coupon-summary p-10">
            ...
            <button class="btn-primary-3d h-50 clip-5 br-5 m-top-20 m-bottom-10 w-full bold"><span>Check Coupon</span></button>
    ...
    <script class="js">
        window.MyFunc = {
            call_back : function(response){
              ...
              let text=`
              <div class="w-full br-5 bg-primary-transparent p-10 column g-10">
